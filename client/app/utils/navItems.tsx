          ...
          navItemsData.map((item, index) => (
            <Link href={`${item.url}`} key={index} passHref>
              <span
                className={`${
                  activeItem === index
                    ? "dark:text-blue-500 text-[crimson]" // Active item styling
                    : "dark:text-white text-black"
                } text-[18px] px-6 font-Poppins font-[400] hover:text-blue-500 dark:hover:text-black transition-colors duration-300`}
              >
                {item.name}
              </span>
            </Link>
